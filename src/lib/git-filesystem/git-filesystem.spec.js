...
describe('git-filesystem (branches)', function () {
    ...
    it('should contain a single master branch after first commit', function (done) {
        gfs.commitAllChanges()
            ...
            .then(function (branches) {
                if (branches.length == 1 && branches.map(b => { return b.name; }).includes('master')) {
                    done();
                } else {
    ...
    it('should contain two branches after creating a new branch', function (done) {
        ...
        gfs.createBranch(branchName)
            .then(function () {
                gfs.getBranches()
                    .then(function (branches) {
                        if (branches.length == 2 && branches.map(b => { return b.name; }).includes(branchName)) {
                            done();
                        } else {
    ...
    it('should allow branched to be created based on an existing named branch.', function (done) {
        gfs.checkoutBranch('newBranch') // Created above
            .then(function () {
                // Add a file
            ...
            .then(function (repo) {
                var commit1;
                var commit2;
                return NodeGit.Reference.nameToId(repo, 'refs/heads/newBranch')
                    .then(function (commit) {
                        commit1 = commit;
                        return NodeGit.Reference.nameToId(repo, 'refs/heads/newBranch2')
                            ...
                            .then(function (masterCommit) {
                                if ((commit1.tostrS() === commit2.tostrS()) && (commit1.tostrS() != masterCommit.tostrS())) {
                                    done();
                                } else {
