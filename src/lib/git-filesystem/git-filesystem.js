                    ...
                    function () {
                        ...
                        return new Promise(function (innerFulfill, innerReject) {
                            NodeGit.Repository.init(repoPath, 0)
                                .then((repo) => { innerFulfill(repo); }, function (err) { innerReject(err); });
                        });
                    })
                .then(function (repo) { repoInstance = repo; fulfill(repo); }, function (err) { reject(err); });
        }
    });
};
    ...
    return new Promise(function (fulfill, reject) {
        getRepository().then(function (repo) {
            repo.getStatus().then(function (statuses) {
                fulfill(statuses.map(s => { return { path: s.path(), status: statusToObj(s) }; }));
            });
        })
            ...
            .then(function (references) {
                for (var i = 0; i < references.length; i++) {
                    if (references[i].isBranch()) {
                        branches.push({ name: references[i].shorthand(), active: (currentBranch == references[i].name()) });
                    }
                }
                branches.sort((a, b) => { return a.name.localeCompare(b.name); });
                fulfill(branches);
            }).catch(function (err) {
                reject(err);
            });
