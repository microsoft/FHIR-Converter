...
var fs = require('fs');
var hl7 = require('../parsers/hl7v2/hl7v2');
    ...
    it('should compile the ADT_A01.hbs template, which includes nested partials (using handlebar new insatnce)', function (done) {
        session.run(() => {
            ...
            fs.readFile(path.join(constants.HL7V2_TEMPLATE_LOCATION, 'ADT_A01.hbs'), function (tErr, templateContent) {
                if (tErr) done(tErr);
                fs.readFile(path.join(constants.HL7V2_DATA_LOCATION, 'ADT01-23.hl7'), function (mErr, messageContent) {
                    if (mErr) done(mErr);

                    var template;
                    try {
                        template = handlebarInstance.compile(templatePreprocessor.Process(templateContent.toString()));
                    } catch (ex) {
                        done(ex);
                    }
        ...
        session.run(() => {
            ...
            fs.readFile(path.join(constants.HL7V2_TEMPLATE_LOCATION, 'ADT_A01.hbs'), function (tErr, templateContent) {
                ...
                fs.readFile(path.join(constants.HL7V2_DATA_LOCATION, 'ADT01-23.hl7'), function (mErr, messageContent) {
                    ...
                    var template;
                    try {
                        template = handlebarExistingInstance.compile(templatePreprocessor.Process(templateContent.toString()));
                        var context = { msg: hl7.parseHL7v2(messageContent.toString()) };
                        JSON.parse(jsonProcessor.Process(template(context)));
                        done();
                    } catch (ex) {
        ...
        session.run(() => {
            ...
            fs.readFile(path.join(constants.HL7V2_TEMPLATE_LOCATION, 'ADT_A01.hbs'), function (tErr, templateContent) {
                ...
                fs.readFile(path.join(constants.HL7V2_DATA_LOCATION, 'ADT01-23.hl7'), function (mErr, messageContent) {
                    ...
                    var template;
                    try {
                        template = handlebarInstanceWithTemplatesMap.compile(templatePreprocessor.Process(templateContent.toString()));
                        var context = { msg: hl7.parseHL7v2(messageContent.toString()) };
