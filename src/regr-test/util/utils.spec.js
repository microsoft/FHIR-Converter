...
describe('Regression test testUtils - compareContent', () => {
    ...
    it ('should compare correctly when given parameters that only different with fields to be deleted', () => {
        const left = testData(), right = testData();
        right.fhirResource.entry[0].resource.date = 'different';
        assert.ok(utils.compareContent(...[left, right].map(JSON.stringify)));
    });
    it ('should throw error when given unequal plain object', () => {
        const left = testData(), right = testData();
        right.fhirResource.entry[0].resource.fullUrl = 'Differen-Url';
        const expectError = {
            name: 'Error',
            message: 'The conversion result has different property: [fhirResource.entry.Array]'
        };
    ...
    it ('should throw error when left object owns more properties than right', () => {
        const left = testData(), right = testData();
        left.fhirResource['newProperty'] = 'new property value';
        const expectError = {
            name: 'Error',
            message: 'The conversion result has these extra properties: [fhirResource.[newProperty]]'
        };
        assert.throws(() => utils.compareContent(...[left, right].map(JSON.stringify)), expectError);
    });
    it ('should throw error when right object owns more properties than left', () => {
        const left = testData(), right = testData();
        right.fhirResource['newProperty'] = 'new property value';
        const expectError = {
            name: 'Error',
            message: 'The conversion result lacks these properties: [fhirResource.[newProperty]]'
        };
        assert.throws(() => utils.compareContent(...[left, right].map(JSON.stringify)), expectError);
    });
