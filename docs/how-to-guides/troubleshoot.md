...
| TemplateCollectionError | DependencyResourceAuthFailed | If using custom templates, this is likely due to one of the following: 1. the service not having a System-Assigned Managed Identity with the "Storage Blob Data Reader" role assignment granted by the Storage Account containing the template; 2. if network isolation is enabled on the Storage Account but the Container Apps environment does not meet the requirements of the network restrictions. | If you think the issue is RBAC-related, ensure that the Container App's System-Assigned Managed Identity is granted the "Storage Blob Data Reader" role assignment by the Storage Account. To verify that this is configured correctly, navigate to the Storage Account, click the "Access Control (IAM)" blade, select "Role Assignments" and ensure that the System-Assigned Managed Identity exists under the "Storage Blob Data Reader" role. If it does not, add this Role Assignment. See more information on Azure Role Assignments [here](https://learn.microsoft.com/en-US/Azure/role-based-access-control/role-assignments). If the issue could be networking-related, ensure that the Container Apps environment is deployed on a Virtual Network, the VNet's subnet has the "Microsoft.Storage" service endpoint enabled, and the Storage Account accepts traffic from the intended subnet. See more information [here](./storage-account-network-isolation.md). |
