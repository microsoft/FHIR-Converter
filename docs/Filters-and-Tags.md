...
| slice_data_by_segments | Given an HL7 v2 message, a segment list and a segment as the end segment (an optional parameter), returns the sliced HL7 v2 message list. <br> Note: The segment in the segment list will be the beginning of the sliced message. If the end segment is provided, all segments after the end segment (including itself) will not be returned. If not, the whole HL7 v2 message will be traversed by default without ending in advance. | Example of parameter preparation: <br> `{% assign segmentList = hl7v2Data \| get_segment_lists: 'segment1' -%}` <br> `{% assign firstSegments = hl7v2Data \| get_first_segments: 'segment2' -%}` <br> After obtaining a segment list and an end segment (optional), use: <br> `{% assign result = hl7v2Data \| slice_data_by_segments: segmentList.segment1, firstSegments.segment2 -%}` <br> or <br>`{% assign result = hl7v2Data \| slice_data_by_segments: segmentList.segment1 -%}` | 
