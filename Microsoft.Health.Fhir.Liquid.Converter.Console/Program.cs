...
namespace Microsoft.Health.Fhir.Liquid.Converter.Tool
{
    public class Program
    {
        ...
        public static void TestCcda()
        {
            var templateDirectory = Path.Join(AppDomain.CurrentDomain.BaseDirectory, Constants.TemplateDirectory, "Ccda");
            CcdaProcessor ccdaProcessor = new CcdaProcessor(new ProcessorSettings(), new ConsoleTelemetryLogger());
            TemplateProvider templateProvider = new TemplateProvider(templateDirectory, DataType.Ccda);

            foreach (var data in GetDataForCcda())
            {
                string rootTemplate = data[0];
                string inputFile = data[1];
                var inputContent = File.ReadAllText(inputFile);
                Console.WriteLine($"input file: {inputFile}.");
                ccdaProcessor.Convert(inputContent, rootTemplate, templateProvider);

                Console.WriteLine(" ");
            }
        }

        public static void TestStu3()
        {
            var templateDirectory = Path.Join(AppDomain.CurrentDomain.BaseDirectory, Constants.TemplateDirectory, "Stu3ToR4");
            FhirProcessor fhirProcessor = new FhirProcessor(new ProcessorSettings(), new ConsoleTelemetryLogger());
            TemplateProvider templateProvider = new TemplateProvider(templateDirectory, DataType.Fhir);

            foreach (var data in GetDataForStu3ToR4())
            {
                string rootTemplate = data[0];
                string inputFile = data[1];
                var inputContent = File.ReadAllText(inputFile);
                Console.WriteLine($"input file: {inputFile}.");
                fhirProcessor.Convert(inputContent, rootTemplate, templateProvider);

                Console.WriteLine(" ");
            }
        }

        public static void TestHl7v2()
        {
            var templateDirectory = Path.Join(AppDomain.CurrentDomain.BaseDirectory, Constants.TemplateDirectory, "Hl7v2");
            Hl7v2Processor hl7v2Processor = new Hl7v2Processor(new ProcessorSettings(), new ConsoleTelemetryLogger());
            TemplateProvider templateProvider = new TemplateProvider(templateDirectory, DataType.Hl7v2);
            Hl7v2TraceInfo hl7v2TraceInfo = new Hl7v2TraceInfo();

            foreach (var data in GetDataForHl7v2())
            {
                string rootTemplate = data[0];
                string inputFile = data[1];
                var inputContent = File.ReadAllText(inputFile);
                Console.WriteLine($"input file: {inputFile}.");
                hl7v2Processor.Convert(inputContent, rootTemplate, templateProvider, hl7v2TraceInfo);

                Console.WriteLine(" ");
            }
        }

        public static IEnumerable<string[]> GetDataForCcda()
        {
        ...
        public static IEnumerable<string[]> GetDataForStu3ToR4()
        {
            var data = new List<string>
            {
        ...
        public static IEnumerable<string[]> GetDataForHl7v2()
        {
            var data = new List<string[]>
            {
